import { DropType, RaindropsOptionsType, UnknownProps } from './Types'
...
export default class Raindrops {
  ...
  options: RaindropsOptionsType = defaultOptions
  ...
  updateDroplets(timeScale: number) {
    if (!this.dropletsCtx || !this.ctx || !this.droplets) return
  ...
  updateDrops(timeScale: number) {
    ...
    this.drops.forEach((drop: DropType, i) => {
      if (!drop.killed) {
        ...
        if (checkCollision) {
          this.drops.slice(i + 1, i + 70).forEach((drop2: DropType) => {
            //basic check
            if (
              drop != drop2 &&
              drop.r > drop2.r &&
